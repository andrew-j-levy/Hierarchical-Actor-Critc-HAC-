"""
Below function designs the training environment.  The designer must provide (i) a Mujoco model (i.e., .xml file), (ii) subgoal space, (iii) state to subgoal projection function (if subgoal is different than state), (iv) initial joint angles/velocities, (v) final goal state space, (vi) goal achievement thresholds, and (vii) maximum episode length.
"""
...
def design_env(show):
    ...
    # Provide ranges for all joint angles and velocities.  The function "reset_sim" in "environment.py" will sample from this space and convert to the state space to get first state.  
    initial_state_space = np.array([[np.pi/4, 7*np.pi/4],[-0.05,0.05]])

    # Provide end goal space.  Supports two types of end goal spaces if user would like to train on a larger end goal space.  For pendulum, it is difficult to specify cartesian position ranges as the pendulum always lies on a circle.  The training goal space is thus hard-coded in the get_next_goal method in "environment.py".
