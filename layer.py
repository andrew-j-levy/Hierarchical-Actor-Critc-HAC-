...
class Layer():
    def __init__(self, layer_number, FLAGS, env, sess, noise_perc = 0.1):
        ...
        self.actor = Actor(sess, env, self.batch_size, self.layer_number)
        ...
        self.subgoal_penalty = -self.time_limit
    ...
    def add_noise(self,action, env):
        ...
        if self.layer_number == 0:
            ...
            action_offset = np.zeros((len(action)))
        else:
    ...
    def get_random_action(self, env):
        ...
        if self.layer_number == 0:
            ...
            action = np.zeros((env.goal_dim))

        # Each dimension of random action should take some value in the dimension's range
        for i in range(len(action)):
            if self.layer_number == 0:
                action[i] = np.random.uniform(-env.action_bounds[i], env.action_bounds[i])
            else:
    ...
    def choose_action(self,agent, env, subgoal_test):
        ...
        # If testing, use policy from target actor network without noise
        if agent.FLAGS.test:

            # return self.actor.get_action(np.reshape(self.current_state,(1,len(self.current_state))), np.reshape(self.goal,(1,len(self.goal))))[0], "Policy"
            return self.actor.get_target_action(np.reshape(self.current_state,(1,len(self.current_state))), np.reshape(self.goal,(1,len(self.goal))))[0], "Target Policy", None

        
        # If subgoal testing, action is output of actor network without noise
        elif subgoal_test: 
            ...
            # If not testing or subgoal testing, choose a noisy or random action.  Then determine whether the subgoal should be tested  
    ...
    def create_prelim_goal_replay_trans(self, hindsight_action, next_state, env):
        ...
        # Create transition evaluating hindsight action for some goal to be determined in future.  Goal will be ultimately be selected from states layer has traversed through.  Transition will be in the form [old state, hindsight action, reward = None, next state, goal = None, finished = None, next state projeted to subgoal]
        transition = [self.current_state, hindsight_action, None, next_state, None, None, env.convert_state_2_subgoal(next_state)]
        ...
        if self.layer_number > 0:
            transition_b = [self.current_state, hindsight_action, 0, next_state, hindsight_action, True, None]
    ...
    def finalize_goal_replay(self,goal_thresholds):
        ...
        for i in range(len(indices)):
            ...
            # if self.layer_number == 3:
                # print("Selected Trans: %d, Index %d" % (i, indices[i]))
    ...
    # Create transition penalizing subgoal if necessary.  The target Q-value when this transition is used will ignore next state as the finished boolena = True.
    ...
    def return_to_higher_level(self, max_lay_achieved, agent, env, attempts_made):
        ...
        if max_lay_achieved is not None and max_lay_achieved >= self.layer_number and (self.layer_number < agent.FLAGS.layers-1 or agent.FLAGS.test):
            return True
        ...
        elif not agent.FLAGS.test and attempts_made >= agent.FLAGS.time_scale:
            return True

        # NOTE: During testing, agent will have env.max_action attempts to achieve goal
        elif agent.FLAGS.test and self.layer_number < agent.FLAGS.layers-1 and attempts_made >= agent.FLAGS.time_scale:
            return True
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        # Reset flag indicating whether layer has ran out of attempts
        self.maxed_out = False

        # Display all subgoals if visualizing training and current layer is bottom layer
        if self.layer_number == 0 and agent.FLAGS.show:
            env.display_subgoals(agent.goal_array)
                
        # Current layer has FLAGS.time_scale attempts to each its goal state. 
        attempts_made = 0       

        while True: 

            # Select action to achieve goal state using epsilon-greedy policy or greedy policy if in test mode
            action, action_type, subgoal_test = self.choose_action(agent, env, subgoal_test)
            ...
            if self.layer_number > 0:
                ...
                # print("Proposed Subgoal: ", action)
                goal_status, max_lay_achieved = agent.layers[self.layer_number - 1].train(agent, env, subgoal_test, episode_num)
            ...
            if goal_status[self.layer_number]:
                ...
                print("Hindsight Goal: ", env.convert_state_2_subgoal(agent.current_state))
            ...
            if self.layer_number == 0:
                ...
                # Otherwise, use subgoal that was achieved in hindsight
                    ...
                    hindsight_action = env.convert_state_2_subgoal(agent.current_state)           
            ...
            if not agent.FLAGS.test:
                ...
                if self.layer_number > 0 and subgoal_test and agent.layers[self.layer_number-1].maxed_out:
            ...
            if (max_lay_achieved is not None and max_lay_achieved >= self.layer_number) or agent.steps_taken >= env.max_actions or attempts_made >= agent.FLAGS.time_scale:
                ...
                if attempts_made >= self.FLAGS.time_scale and not goal_status[self.layer_number]:
                    self.maxed_out = True
